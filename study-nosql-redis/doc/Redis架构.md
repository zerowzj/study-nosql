...
​		Redis Sentinel是Redis高可用的实现方案。Sentinel是一个管理多个Redis实例的工具，主要功能包括主节点存活检测、主从运行情况检测、自动故障转移（failover）、主从切换。Redis的Sentinel最小配置是一主一从。 Redis的Sentinel系统可以用来管理多个Redis服务器，该系统可以执行以下四个任务：

1. 监控

   不断的检查主服务器和从服务器是否正常运行
...
​		Redis在3.0版本以后开始支持集群，经过中间几个版本的不断更新优化，最新的版本集群功能已经非常完善。
